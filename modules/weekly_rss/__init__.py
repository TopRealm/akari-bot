...
async def weekly_rss(fetch: Bot.FetchTarget, ctx: Bot.ModuleHookContext):
    ...
    weekly_cn = [
        Plain(
            Locale('zh_cn').t(
                'weekly_rss.message.ysarchives_weekly_rss',
                prefix=command_prefix[0]) +
            weekly)]
    weekly_tw = [
        Plain(
    ...
    post_msg = {'zh_cn': weekly_cn, 'zh_tw': weekly_tw, 'fallback': weekly_cn}
    await fetch.post_message('ysarchives_weekly_rss', I18NContext(post_msg), i18n=True)
    Logger.info('YsArchives Biweekly checked.')
